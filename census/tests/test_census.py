...
from src.census import RunMetadata, SweepConfig, measure_checkpoint, run_sweep
...
def measured():
    ...
    return result, metadata, torch.as_tensor(eval_data.labels)


def test_census_rows_include_layer_position_and_initialization_metadata(measured):
    result, metadata, labels = measured
    frame = measure_checkpoint(result, result.checkpoints[-1], result.checkpoints[0], metadata, labels)
...
def test_initialization_is_its_own_zero_excess_baseline(measured):
    result, metadata, labels = measured
    initial = measure_checkpoint(result, result.checkpoints[0], result.checkpoints[0], metadata, labels)
...
def test_failed_run_is_rejected_from_census(measured):
    ...
    with pytest.raises(ValueError, match="excluded"):
        measure_checkpoint(result, result.checkpoints[-1], result.checkpoints[0], metadata, labels)
    result.passed = True


def test_small_sweep_writes_tidy_outputs_and_omits_fixed_relu(tmp_path):
    config = SweepConfig(
        datasets=("blobs",),
        depths=(2,),
        widths=(5,),
    ...
    census, status, dynamics = run_sweep(config, tmp_path)
    assert len(status) == 2
    assert status["passed"].all()
    assert dynamics.empty
    assert len(census[census["activation"] == "tanh"]) == 2 * 9
    relu = census[census["activation"] == "relu"]
    assert len(relu) == 2 * 6
    assert not relu["format"].str.startswith("fixed-").any()
    assert (tmp_path / "saturation.csv").is_file()
    assert (tmp_path / "saturation.parquet").is_file()
    assert (tmp_path / "training_status.csv").is_file()
    assert (tmp_path / "training_status.parquet").is_file()

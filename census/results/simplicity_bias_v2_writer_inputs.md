# Track T v2 (simplicity bias, redesign) writer inputs

**Status, 02:49 EDT: gate PASS, registration NOT made, no training.**
- The registered ∇ρ₂ check failed at the freeze, so κ_q is undefined.
- Sources:
  - `results/simplicity_bias_v2_design.md` (design, amendment, gate verdict, freeze record);
  - `results/simplicity_bias_v2/pilot_summary.json` and `pilot_scan.csv`;
  - `results/simplicity_bias_v2/frozen.json` (SHA-256 deb8853e…; a record, not a registration);
  - `results/simplicity_bias_v2/lambda_rule.json`.
- v1 (gate FAIL): `results/simplicity_bias_design.md`.

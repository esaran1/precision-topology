# Track T v3 (registered test at the attained switch) writer inputs

**Verdict: UNRESOLVED, on both validity conditions** (too few crossings and χ too large). C1 and C2 are therefore
UNRESOLVED; their would-be verdicts are FAIL and FAIL.

Sources:
- Registration: `results/simplicity_bias_v3_registration.md`, commit `91ef9cf`, made before any training.
- Frozen inputs: `simplicity_bias_v3/frozen_inputs.json`, SHA-256 430e1ead….
- Results: `simplicity_bias_v3/score.json`, `runs_summary.csv` and `runs/` (every step's parameters).
- Producer: `src/simplicity_bias_v3.py`.
